...
function utils.diff_str(str1, str2, key)
    ...
    if typ1 ~= typ2 then
        return 'type<br /><code>'..tostring(typ1)..'</code>&nbsp;vs.&nbsp;<code>'..tostring(typ2)..'</code>', key or 0
    end
        ...
        if str1:sub(i, i) ~= str2:sub(i, i) then
			...
			for j = 1, max2 do
				if tempStr1:sub(j, j) ~= tempStr2:sub(j, j) then
					return (key and tostring(key) or tostring(j))..'<br />"<code>'..tempStr1:sub(j, j)..'</code>"&nbsp;vs.&nbsp;"<code>'..tempStr2:sub(j, j)..'</code>"', key or j
				end
			end
			
			return (key and tostring(key) or tostring(i))..'<br />"<code>'..str1:sub(i, i)..'</code>"&nbsp;vs.&nbsp;"<code>'..str2:sub(i, i)..'</code>"', key or i
        end
    end
    
    max = max + 1;
    return (key and tostring(key) or tostring(max))..'<br />"<code>'..str1:sub(max, max)..'</code>"&nbsp;vs.&nbsp;"<code>'..str2:sub(max, max)..'</code>"', key or max
end
...
function utils.diff_tbl(tbl1, tbl2, key)
    ...
    local ty2 = type(tbl2)
    if ty1 ~= ty2 then
        return key or 'type<br /><code>'..tostring(ty1)..'</code>&nbsp;vs.&nbsp;<code>'..tostring(ty2)..'</code>'
    end
    local OTHER_TYPES = {
        ['boolean'] = 1,
        ['nil']     = 1,
...
local function render_report(suite_status, total_tests, failed_tests, config, failed_members, time_exec, test_results, member_fails)
    ...
    report:tag('ul')
    	:tag('li')
            :wikitext('<code>' .. i18n:msg('report-time') .. ': ' .. tostring(math.floor(time_exec * 10 + 0.5) / 10) .. ' ms</code>')
            :done()
    	:tag('li')
            :wikitext('<code>' .. i18n:msg('report-failing') .. ':</code>' .. (#failed_members ~= 0
                    and '<ul><li><code>' .. table.concat(failed_members, '</code></li><li><code>') .. '</code></li></ul>'
                    .. '[[Category:Failing Lua test suites]]'
                    or  '<code> ' .. i18n:msg('report-none') .. '</code>'
                ))
            :done()
        :tag('li')
			:wikitext('<code>' .. i18n:msg('report-missing') .. ':</code>' .. ((#config.neg - mCount) ~= 0
                and '<ul><li><code>' .. table.concat(missed, '</code></li><li><code>') .. '</code></li></ul>'
                or  '<code> ' .. i18n:msg('report-none') .. '</code>'
            ))
            :done()
                    ...
                    :node(config.display == false and test_output.options.nowiki ~= false and i ~= 0
                        and mw.html.create('td')
                            ...
                            :wikitext(
                                ...
                                '<span style="background-color: #01872e; padding: 0.25em 0.1em; border-radius: 0.33em; color: white">'..
                                member_nowiki(type(test_result.expect) == 'table'
                                    and inspect(test_result.expect, inspect_options)
                    ...
                    :node(config.display == false and test_output.options.nowiki ~= false and i ~= 0
                        and mw.html.create('td')
                            ...
                            :wikitext(
                                ...
                                '<span style="background-color: #e6173c; padding: 0.25em 0.1em; border-radius: 0.33em; color: white">'..
                                member_nowiki(type(test_result.actual) == 'table'
                                    and inspect(test_result.actual, inspect_options)
