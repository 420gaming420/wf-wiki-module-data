...
function p.getAbility(frame)
	...
	table.insert(out, '<h4 style="font-size:2em">' .. abilityID .. '</h4>')
...
function p.helminth_list(frame)
	...
	if notes > 0 then
		table.insert(out, "<b>Editor notes</b>")
		table.insert(out, '<references group="fn"/>')
	end

	if credits > 0 then
		table.insert(out, "<b>Credits</b>")
		table.insert(out, '<references group="ty"/>')
	end
					...
					if augments then
						...
						for _, augName in ipairs(augKeys) do
							...
							if vol == targetVolume or revised == targetVolume or retired == targetVolume then
								...
								table.insert(out, "|description=<b>" ..
									abilityID ..
									" augment:</b> " ..
...
function p.augment_tier_list(frame)
	...
	table.insert(out, '* <b>New Augments:</b> ' .. total_custom .. ' (' .. (total_custom - helminth_custom) .. ' excluding Helminth)')
	table.insert(out, '* <b>Revised Official Augments:</b> ' .. revised_augments)
	table.insert(out, '* <b>Revised Abilities:</b> ' .. revised_abilities)
	...
	table.insert(out, '* <b>Other Contributors:</b> ' .. total_unique_contributors)
	table.insert(out, "<div style='margin-left:2em; column-width:200px; max-width:1000px;'>")
	...
	table.insert(out, '<table class="wikitable sortable">')
	table.insert(out, "<tr><th>Augments</th><th>Warframes</th></tr>")
	...
	for i = max_count, 0, -1 do
		...
		table.insert(out, "<tr><td>" .. i .. "</td><td>" .. table.concat(wfs_html, " • ") .. "</td></tr>")
	end
