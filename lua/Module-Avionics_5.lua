...
tooltipText = function(frame)
    ...
    if Name == nil then
        ...
        return '<span style="color:var(--negative-text-color);">{{[[Template:Avi]]}}</span> Insert an avionic name.['..'[Category:Avionic Tooltip error]]'
    end
    
    local Avionic = getAvionic(Name)
    if Avionic == nil then
        --error(Name..' not found')
        return '<span style="color:var(--negative-text-color);">{{[[Template:Avi|Avionic]]}}</span> "'..Name..'" not found['..'[Category:Avionic Tooltip error]]'
    end
...
tooltip = function(frame)
    ...
    ..'\n|-\n| style="background-color: var(--tt-bg-color); color: var(--almost-white); padding: 10px; font-size:13px; line-height: 16px;"|<span style="font-weight: bold;font-size: 14px;">'
