---
title: "Module:DropTables/dev"
wiki_url: "https://wiki.warframe.com/w/Module/DropTables/dev"
wiki_timestamp: "2022-03-18T00:55:39Z"
---

**Lua error at line 78: attempt to perform arithmetic on field '?' (a string value).**
**Lua error at line 78: attempt to perform arithmetic on field '?' (a string value).**
**Lua error at line 236: attempt to call field 'getFullTooltip' (a nil value).**
**Lua error at line 223: attempt to call field 'getFullTooltip' (a nil value).**
<infobox>
...
```lua
       ...
       [[File:{{#setmainimage:Mod.png}}|246px]]
       [[Lua error in Module:Mods at line 53: p.getValue(frame): No such mod "" found (Did you make a typo? If not, may need to add a new entry to Module:Mods/data).|246px]]
```

<default>

[Untransmutable](/w/Transmutation "Transmutation")
...
```lua
       ...
       Statistics
           ...
           Trading Tax
           ![](/images/thumb/Credits64.png/20px-Credits64.png?6022f)
           8,000
...
local function formatDropString(drop)
	...
	elseif (dropType == "Relic") then
		...
		-- Mimicing frame object in order to replace displayed text with " (Radiant)"
...
" end
			...
			for i, Drop in pairs(Drops) do
				result = result.."  
"..linkEnemy(Drop.EnemyName)..space
...
', ': '):gsub(' ', ' ')))
	...
	return table.concat(missionList, '  
'), table.concat(enemyList, '  
'), table.concat(containerList, '  
')
end
