---
title: "Module:Text"
wiki_url: "https://wiki.warframe.com/w/Module/Text"
wiki_timestamp: "2026-01-06T06:46:24Z"
---

**Text** creates a wrapper to simplify and standardize markup for text inside  tags for use on articles and within modules.

On this Wiki, Text is used in:

* [Template:Text](/w/Template:Text "Template:Text")
...
`{{#invoke:Text|main|input1|input2|...}}`
...
`text._text(text, args, class, color, backgroundColor, hoverText, hoverColor, noWrap, cursor, fontFamily)` (function)
:   Creates a  element with the specified attributes set.
:   **Parameters**:

    * `text` Displayed text between  tags (string)
    * `args` Containing optional named arguments for the below: (table; optional)
    * `class` CSS class for  element (string; optional)
    * `color` Color of text for color attribute (string; optional)
    * `backgroundColor` Background color of  for background-color attribute (string; optional)
    * `hoverText` Displayed text on hover over, title attribute value (string; optional)
    * `hoverColor` Color of underlined bottom border (string; optional)
    * `noWrap` Prevents text to wrap to next paragraph (boolean; optional)
    * `cursor` Cursor type on hover over (string; optional)
    * `fontFamily` Font family of displayed text (string; optional)
:   **Returns**: Output  element (string)

`text.main(frame)` (function)
:   Invoked within templates to create formatted  elements.
:   **Parameter**: `frame` Frame object (object)
:   **Returns**: Output  element (string)
...
* [Text/doc](/w/Module:Text/doc "Module:Text/doc")
...
```lua
---	'''Text''' creates a wrapper to simplify and standardize markup for text inside  tags
--	for use on articles and within modules.  
...
--- Creates a  element with the specified attributes set.
--	@function		p._text
--	@param			{string} text Displayed text between  tags
--	@param[opt]		{table} args Containing optional named arguments for the below:
--	@param[opt]		{string} class CSS class for  element
--	@param[opt]		{string} color Color of text for color attribute
--	@param[opt]		{string} backgroundColor Background color of  for background-color attribute
...
function p._text(text, args)
	...
	-- Final HTML
	return string.format( '%s', classStr, styleStr, titleStr, textStr )
end
