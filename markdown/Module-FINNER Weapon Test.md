---
title: "Module:FINNER Weapon Test"
wiki_url: "https://wiki.warframe.com/w/Module/FINNER Weapon Test"
wiki_timestamp: "2021-06-09T18:08:33Z"
---

*Documentation for this module may be created at [Module:FINNER Weapon Test/doc](/w/Module:FINNER_Weapon_Test/doc?action=edit&redlink=1 "Module:FINNER Weapon Test/doc (page does not exist)")*
...
local function makeTooltip(dataParam, image, link)
	return string.format('[[File:%s|x19px]] [[%s]]',
		dataParam,
		image or 'Panel.png',
		link
	)
...
local function polarity(pol)
    ...
    })[pol]or(pol)] or error('Could not find polarity: '..(pol or '')))
end
...
buildInfobox = function(frame)
	...
	if dispo then
    	...
    	dispo = ('%s%s (%.2fx)'):format(('●'):rep(dispo), ('○'):rep(5-dispo), tonumber(override(args.Disposition)) or weapon.Disposition)
    end
	...
	local infobox = mw.html.create('infobox')
		...
		:tag('group'):attr('collapse', 'open')
			...
			:tag('data'):attr('source', 'Zoom')
				...
				:tag('default'):wikitext(weapon.Zoom and table.concat(weapon.Zoom, '  
')):done()
			:done()
