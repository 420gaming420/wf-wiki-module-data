---
title: "Module:VoidByReward"
wiki_url: "https://wiki.warframe.com/w/Module/VoidByReward"
wiki_timestamp: "2021-06-16T18:42:35Z"
---
...
All of this module's functions have been moved to [Module:Void](/w/Module:Void "Module:Void") and reworked. This page is kept up for archival reasons.
...
local Shared = require( "Module:Shared" )	-- TODO: Replace M:Shared with M:Table and move Shared.relicLoop() here or in M:Void
...
function p.byReward(frame)
    ...
    for _, relic in pairs(VoidData["Relics"]) do
        for i, drop in pairs(relic.Drops) do
            local newObj = {Tier = relic.Tier, Name = relic.Name, Rarity = drop.Rarity, Vaulted = relic.Vaulted ~= nil, IsBaro = relic.IsBaro == 1}
    ...
    local fullTable = {'{|class="bigmodtable" style="line-height:16px; font-size:14px;"'}
    ...
    for item, parts in Shared.skpairs(data) do
        ...
        local image = Icon._Prime(item, nil, "64")
...
")
                ...
                table.insert(fullTable,(ttStart..dropTier..' '..dropName..ttCenter.."[["..dropTier..' '..dropName.."]]"..ttEnd.." "..dropRarity))
                
                if(drop.Vaulted ~= nil) then
...
function p.simpleRewardTable(frame)
    ...
    for _, relic in pairs(VoidData["Relics"]) do
        ...
        for i, drop in pairs(relic.Drops) do
            ...
            local thisObj = {Item = drop.Item, Part = drop.Part, Tier = relic.Tier,
                            Name = relic.Name, Rarity = drop.Rarity, Vaulted = vault}
    ...
    local strTable = {'{| class="article-table sortable" style="width: 600px;margin: auto;" cellspacing="1" cellpadding="1" border="2"'}
    local col1 = '\n|-'
    local col2 = '\n! scope="col"|Item'
    local col3 = '\n! scope="col"|Part'
    local col4 = '\n! scope="col"|Tier'
    ...
    local col6 = '\n! scope="col"|Rarity'
    local col7 = '\n! scope="col"|Vaulted?'
    table.insert(strTable, (col1..col2..col3..col4..col5..col6..col7))
    
    for i, Row in pairs(data) do
        local ItemName = '[['..Void.getItemName(Row.Item)..']]'
        local PartName = Void.getPartName(Row.Part)
        local rowStr = '\n|- \n|'..ItemName..'||'..PartName..'||'..Row.Tier..'||'..ttStart..Row.Tier..' '..Row.Name..ttCenter..'[['..Row.Tier..' '..Row.Name..'|'..Row.Name..']]'..ttEnd..'||'..Row.Rarity..'||'..(Row.Vaulted)
...
function p.relicsTable(frame)
    ...
    local tableStr = {"{| class=\"article-table\" cellspacing=\"1\" cellpadding=\"1\" border=\"2\" style=\"margin:auto\""}
    ...
    for tier in Shared.relicLoop() do
        ...
        for i, relic in pairs(data[tier]) do
            ...
            table.insert(tableStr, ('\n*'..ttStart..relicText..ttCenter.."[["..relicText.."]]"..ttEnd))
        end
    end
...
function p.byRelic(frame)
    ...
    for i, relic in pairs(VoidData["Relics"]) do
        ...
        --Tier & Relic are easy
        table.insert(tableStr, ("\n| "..relic.Tier))
        table.insert(tableStr, ("\n| ".."[["..relic.Tier.." "..relic.Name.."|"..relic.Name.."]]"))
...
function p.byRarity(frame)
    ...
    local dataIndex = {["Common"] = 1, ["Uncommon"] = 2, ["Rare"] = 3}
    local data = {[1] = {}, [2] = {}, [3] = {}}
    ...
    for _,relic in pairs(VoidData["Relics"]) do
        ...
        if(tier == checkTier) then
            for _, drop in pairs(relic.Drops) do
                ...
                if(data[dataIndex[rarity]][item.."|"..part] == nil) then
                    data[dataIndex[rarity]][item.."|"..part] = 1
                end
            end
        end
    end
    ...
    for rarity, _ in ipairs(data) do
        --1. Loop through each item in that rarity
        for itemStr, _ in Shared.skpairs(data[rarity]) do
            --2. Get the item & part
            local sp1, _ = string.find(itemStr, "|")
        ...
        if (rarity ~= 3) then table.insert(tableStr, "\n|") end
    end
