---
title: "Module:Flaicher/Mods"
wiki_url: "https://wiki.warframe.com/w/Module/Flaicher/Mods"
wiki_timestamp: "2019-03-27T07:17:53Z"
---

*Documentation for this module may be created at [Module:Flaicher/Mods/doc](/w/Module:Flaicher/Mods/doc?action=edit&redlink=1 "Module:Flaicher/Mods/doc (page does not exist)")*
    ...
    elseif(UpName == "TRANSMUTABLE") then
        if(Mod.Transmutable ~= nil) then
            if(Mod.Transmutable) then
                return '[[Transmutation|Transmutable]]'
            else
                return '[[Transmutation|Untransmutable]]'
            end
        else
            return '[[Transmutation|Transmutable]]'
        end
...
function p.tooltipText(frame)
    ...
    local getActualIcon = frame.args["Actual"] == "true" and true
    local simpleIcon = frame.args["Simple"] == "true" and true
    local Mod = getMod(modName)
    local rarityList = {'Peculiar','Common','Uncommon','Rare','Legendary','Riven','N/A'}
    local link = ''
    local fullLink = ''
    local errorCat = '[[Category:Mod'..' Tooltip error]]'
    
    if ifModExists(modName, Mod) == false then
        return '[['..modName..']]'..errorCat
    end
    
    if Mod.Link ~= nil then
        link = Mod.Link
        fullLink = '[['..Mod.Link..'|'..Mod.Name..']]'
    else
        link = Mod.Name
        fullLink = '[['..Mod.Name..']]'
    end
    
    if getActualIcon == true and Mod.Image ~= nil then
        return '[[File:'..Mod.Image..'|x19px|link='..link..']] '..fullLink..''
    elseif simpleIcon == true then
        return '[[File:Mod minimap icon.png|x19px|link='..link..']] '..fullLink..''
    elseif Shared.contains(rarityList, Mod.Rarity) then
        local image = ''
        if Mod.Rarity == 'Peculiar' then
            image = 'Generic_Mod_Icon_Peculiar.png'
        elseif Mod.Rarity == 'Common' or Mod.Rarity == 'N/A' then
            image = 'Generic_Mod_Icon_Common.png'
        elseif Mod.Rarity == 'Uncommon' then
            image = 'Generic_Mod_Icon_Uncommon.png'
        elseif Mod.Rarity == 'Rare' then
            image = 'Generic_Mod_Icon_Rare.png'
        elseif Mod.Rarity == 'Legendary' then
            image = 'Generic_Mod_Icon_Legendary.png'
        elseif Mod.Rarity == 'Riven' then
            image = 'Generic_Mod_Icon_Riven.png'
        end
        return '[[File:'..image..'|x19px|link='..link..']] '..fullLink..''
    else
        return '[['..Mod.Name..']]'..errorCat
    end
end
