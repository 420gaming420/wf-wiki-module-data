...
local function byRank(r0, r1, r2, r3)
	return ('%s'):format(r0, r1, r2, r3, r3)
end
...
', u='when at max heat:'}, {v='1t', u='/s-2  
t = time at max heat in seconds'}, }, -- WIP? too long?
	['Inferno'      ] = { {v=true}, {v=10, u='/enemy  
(max 10x)'}, }, -- WIP?
	['Mach Rush'    ] = { {v=true}, {v=12.5, u='/s'}, },
	['Mass Vitrify' ] = { {v=true}, {v='3-5', u='/s'}, },
	['Mend & Maim'  ] = { {v=true}, {v=3.5, u='/s'}, },
	['Navigator'    ] = { {v=true}, {v=3, u='/s'}, {v=2, u='/s2'}, },--:gsub('%^([0-9-]+)','%1')
...
buildInfobox = function(frame)
	...
	drain = table.concat(drain,'  
+'):gsub('  
%+(%[%[[^<]-<[^>]->)%-', '  
-%1')
	if drain:find('>0[/b]+]]$') then drain = '' end
...
\n\n')
	...
	local abilityid=ability.InternalName and ability.InternalName:match('/%w+$') or ''
...
\n{{#lsth:%s|Characteristics}}")
	...
	local strength_tip = Tooltips.full{'Ability Strength', 'Stats', r='Strength'}
	local duration_tip = Tooltips.full{'Ability Duration', 'Stats', r='Duration'}
	local range_tip    = Tooltips.full{'Ability Range',    'Stats', r='Range'   }
	local efficiency_tip=Tooltips.full{'Ability Efficiency','Stats',r='Efficiency'}

	local ability_tip = Tooltips.text { { 'Ability', name, link, _data=ability } }

	-- Old version deprecated since 2022 (see [[User_blog:Cephalon_Scientia/Sunsetting_of_Maximization_Calculators]])
	-- New DSL since 2026
	local max = Maximization.main(name)
	if max then max = '==Work in progress==\n'..max end
...
'|width=30% style="padding-right: 6px; font-size: 14px; vertical-align: top;"|

'..frame:callParserFunction('#tag:tabber',sub)..'

\n\n|-\n|\n|-\n|\n|-\n'
or ([=[
|width=30%% style="padding-right: 6px; font-size: 14px; border-bottom:1px solid;"|'''%s:'''%s
|-
| style="padding-right: 6px; font-size: 14px; border-bottom:1px solid;"|'''%s:'''%s
|-
| style="padding-right: 6px; font-size: 14px;"|'''%s:'''%s
|-
%s
%s]=]):format(
strength_tip, strength ~= '' and strength or 'N/A',
duration_tip, duration ~= '' and duration or 'N/A',
range_tip   , range    ~= '' and range    or 'N/A',

efficiency ~= '' and efficiency and ([=[| style="padding-right: 6px; font-size: 14px; border-top:1px solid;" |'''%s:'''
%s
|-]=]):format(efficiency_tip, efficiency) or '',
...
nil)
	...
	local keybindLink = ([=[[[Key Bindings|%s]]]=]):format(hotkey ~= '' and hotkey or 'N/A')
...
|rowspan=5 style="width: auto; text-align: center; max-width: 20%%;"|[[File:%s|x50px|link=%s|class=light-invert]]  
%s  
%s
|rowspan=5|%s  
