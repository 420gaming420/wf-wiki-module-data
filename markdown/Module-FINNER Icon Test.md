---
title: "Module:FINNER Icon Test"
wiki_url: "https://wiki.warframe.com/w/Module/FINNER Icon Test"
wiki_timestamp: "2021-03-22T23:29:58Z"
---

*Documentation for this module may be created at [Module:FINNER Icon Test/doc](/w/Module:FINNER_Icon_Test/doc?action=edit&redlink=1 "Module:FINNER Icon Test/doc (page does not exist)")*

```lua
--WARFRAME Wiki Icon Module
--https://warframe.fandom.com/wiki/Template:Icon
...
function p._Item(iconname, textexist, imagesize)
    ...
    if IconData["Items"][iconname] == nil then
        return "Invalid[[Category:Icon Module error]]"
    else
...
function p._Pol(iconname, color, imagesize)
    if IconData["Polarities"][iconname] == nil then
        ...
        if iconname == "O" or iconname == "o" then
            return '[[File:'..iconname..'|'..imagesize..'px|link=Polarity]][[Category:IconORename]]'
        end
...
function p._Dis(name, color, size)
    ...
    if name  == nil then
        ...
        elseif(name <= 1.1) then dots = '●●●○○'
        elseif(name <= 1.3) then dots = '●●●●○'
        else                     dots = '●●●●●' end
        return '[['..link..'|'..dots..']]'
    end
end
...
function p._Prime(primename, partname, imagesize)
    ...
    if IconData["Primes"][primename] == nil then
        return 'Icon not found[[Category:Missing icon]]'
    end
...
function p._Proc(iconname, textexist, color, ignoreTextColor,noTooltip)
    ...
    elseif IconData["Procs"][iconname] == nil then
        ...
        if (textexist == 'text' or textexist == 'Text') then
            ...
            elseif (IconData["Procs"][iconname]["color"] ~= nil) then
                textcolor = IconData["Procs"][iconname]["color"]
            end

            if(ignoreTextColor == nil or not ignoreTextColor) then
                if(tooltip ~= nil and tooltip ~= '') then
                    return span1..'[[File:'..iconFile..'|link='..link..'|'..tooltip..']] [['..link..'|'..iconname..']]'..span2
                else
                    return span1..'[[File:'..iconFile..'|link='..link..']] [['..link..'|'..iconname..']]'..span2
                end
            else
                if(tooltip ~= nil and tooltip ~= '') then
                    return span1..'[[File:'..iconFile..'|link='..link..'|'..tooltip..']] [['..link..'|'..iconname..']]'..span2
        ...
        if(tooltip ~= nil and tooltip ~= '') then
            return span1..'[[File:'..iconFile..'|link='..link..'|'..tooltip..'|alt='..iconname..']]'..span2
        else
            return span1..'[[File:'..iconFile..'|link='..link..']]'..span2
        end
    end
end
...
function p._Clan(iconname, imagesize)
    ...
    if IconData["Clan"][iconname] == nil then
        return "Invalid"
    else
        ...
        return '[[File:'..iconFile..'|'..imagesize..'px|link='..link..'|'..iconname..' Clan]]'
    end
end

local function tooltipCheck(name, typename)
    local procList = {"Impact","Puncture","Slash","Cold","Electricity","Heat","Toxin","Void","Tau","Blast","Corrosive","Gas","Magnetic","Radiation","Viral","True","Ballistic","Plasma","Particle","Frost","Ionic","Incendiary",{"Chem","Chemical"}}
