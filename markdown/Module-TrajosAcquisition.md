---
title: "Module:TrajosAcquisition"
wiki_url: "https://wiki.warframe.com/w/Module/TrajosAcquisition"
wiki_timestamp: "2022-01-04T18:17:02Z"
---

*Documentation for this module may be created at [Module:TrajosAcquisition/doc](/w/Module:TrajosAcquisition/doc?action=edit&redlink=1 "Module:TrajosAcquisition/doc (page does not exist)")*
...
|-]]);
    ...
    for i, v in ipairs(itemData) do
    	...
    	local day = 'Day ';
    	local dayAspect = 'Day Aspect';
    	local night = 'Night ';
    	local nightAspect = 'Night Aspect';
        for j, w in ipairs(v) do
        	...
        	if part ~= nil and part ~= '' then
				...
				if part == 'Main Blueprint' then
	        		...
	        		if part ~= dayAspect and string.sub(part, 1, #day) == day  then
	        			part = string.sub(part, 1 + #day)
	        		elseif part ~= nightAspect and string.sub(part, 1, #night) == night then
	        			part = string.sub(part, 1 + #night)
	        		end
	        		
	        		if IconData["Items"][part] ~= nil then
	        			icon = Icon._Item(part, nil, nil)..' ';
	        		end
	        	end
        	end
        	
            table.insert(result, '| '..icon..(w.Mission or w.Part)..((w.Desc ~= nil and w.Desc ~= '') and '  
('..w.Desc..')' or ''));
