---
title: "Module:Weapons/data/validate"
wiki_url: "https://wiki.warframe.com/w/Module/Weapons/data/validate"
wiki_timestamp: "2026-06-21T03:41:35Z"
---
...
local TYPE_ERROR_MSG = '# "[[%s]]" contains a %s type instead of a %s type for %s'
local UNSUPPORTED_TYPE_ERROR_MSG = '# "[[%s]]" contains an unsupported key %s'
...
function p.validateDataTypes(frame)
	local weaponErrors = { 'p.validateDataTypes(frame): There are a total of %d key-value errors' }
...
function p.checkForMissingData(frame)
	local weaponErrors = { 'p.checkForMissingData(frame): There are a total of %d key-value errors' }
	for key, weapon in Table.skpairs(WeaponData) do
		if (weapon.Attacks == nil) then
			table.insert(weaponErrors, '# "[['..key..']]" does not have Attacks key')
		end
		if (weapon.Attacks ~= nil and weapon.Attacks[1].Damage == nil) then
			table.insert(weaponErrors, '# "[['..key..']]" does have Attacks key but Attacks[1].Damage is nil')
		end
		...
		for i, attack in ipairs(weapon.Attacks or {}) do
			for attackKey, _ in pairs(p._REQUIRED_ATTACK_STATS) do
				if (attack[attackKey] == nil) then
					table.insert(weaponErrors, '# "[['..key..']]" Attacks['..i..'].'..attackKey..' is nil')
				end
			end
		end
		if (weapon._TooltipAttackDisplay ~= nil and weapon.Attacks[weapon._TooltipAttackDisplay] == nil) then
			table.insert(weaponErrors, '# "[['..key..']]" _TooltipAttackDisplay is referencing a non-existent '..weapon._TooltipAttackDisplay..'')
		end
		if (weapon.Class == nil) then
			table.insert(weaponErrors, '# "[['..key..']]" does not have a Class key')
		end
		if (weapon.Disposition == nil and DISPOSITION_MASTERY_EXEMPTIONS_SLOTS[weapon.Slot] == nil and DISPOSITION_MASTERY_EXEMPTIONS_CLASSES[weapon.Class] == nil) then
			table.insert(weaponErrors, '# "[['..key..']]" does not have Disposition key')
		end
		if (weapon.Image == nil) then
			table.insert(weaponErrors, '# "[['..key..']]" does not have an Image key')
		end
		if (weapon.Introduced == nil) then
			table.insert(weaponErrors, '# "[['..key..']]" does not have Introduced key')
		end
		if (weapon.Link == nil) then
			table.insert(weaponErrors, '# "[['..key..']]" does not have Link key')
		end
		if (weapon.Mastery == nil and DISPOSITION_MASTERY_EXEMPTIONS_SLOTS[weapon.Slot] == nil and DISPOSITION_MASTERY_EXEMPTIONS_CLASSES[weapon.Class] == nil) then
			table.insert(weaponErrors, '# "[['..key..']]" does not have Mastery key')
		end
		if (weapon.Name == nil) then
			table.insert(weaponErrors, '# "[['..key..']]" does not have Name key')
		end
		if (weapon.Slot == nil) then
			table.insert(weaponErrors, '# "[['..key..']]" does not have a Slot key')
		end
	end
	weaponErrors[1] = string.format(weaponErrors[1], #weaponErrors - 1)
	return frame:preprocess(table.concat(weaponErrors, '\n'))
...
function p.validateAttacks(frame)
	local weaponErrors = { 'p.validateAttacks(frame): There are a total of %d key-value errors' }
	for weaponName, weapon in Table.skpairs(WeaponData) do
		for _, attack in pairs(weapon.Attacks) do
			for key, validateFunction in pairs(p._REQUIRED_ATTACK_STATS) do
				...
				if (not pass) then
					table.insert(weaponErrors, '# "[['..weaponName..']]" Attack'..i..': '..message)
				end
			end
		end
	end
...
function p.validateRequiredKeys(frame)
	local weaponErrors = { 'p.validateRequiredKeys(frame): There are a total of %d key-value errors' }
	for weaponName, weapon in Table.skpairs(WeaponData) do
