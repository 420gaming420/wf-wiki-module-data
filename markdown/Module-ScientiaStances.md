---
title: "Module:ScientiaStances"
wiki_url: "https://wiki.warframe.com/w/Module/ScientiaStances"
wiki_timestamp: "2021-10-25T17:13:28Z"
---

*Documentation for this module may be created at [Module:ScientiaStances/doc](/w/Module:ScientiaStances/doc?action=edit&redlink=1 "Module:ScientiaStances/doc (page does not exist)")*
...
local function validateStanceData(stanceName)
    ...
    local errorBegin = 'Error: '..stanceName..' combo '
    local errorEnd = ' in Module:ScientiaStances/data'
...
local function buildTopProcIconHTML(proc)
    ...
    if (type(proc) == 'string') then
        if (SHARED.contains(PROC_NAMES, proc)) then
            return ''..tooltip.getFullTooltip(proc, "DamageTypes")..''
...
local function buildHitInfoHTML(multiplier, numberOfHits, proc)
    ...
    if (type(multiplier) == 'number' and multiplier ~= 1) then
        ...
        if (numberOfHits > 1) then
            HTMLresult = HTMLresult..''..numberOfHits..'x '..(multiplier * 100)..'%'
        
        -- for a single hit
        else
            HTMLresult = HTMLresult..''..(multiplier * 100)..'%'
        end
    ...
    elseif (numberOfHits ~= nil and numberOfHits > 1) then
        HTMLresult = HTMLresult..''..numberOfHits..'x'
    end
    return HTMLresult
end
...
function p.addComboIcon(attackType, procType, multiplier, numberOfHits, proc)
    ...
    --             comboIcon = comboIcon..''..numberOfHits[1]..'x '..(multiplier[1] * 100)..'%'
    --             comboIcon = comboIcon..''..numberOfHits[2]..'x '..(multiplier[2] * 100)..'%'
    --         end
    --     -- for only single damage multiplier
    --     else
    --         -- for a single hit
    --         if (numberOfHits == 1) then
    --             comboIcon = comboIcon..''..(multiplier * 100)..'%'
    --         -- for multiple hits with the same multiplier
    --         else
    --             comboIcon = comboIcon..''..numberOfHits..'x '..(multiplier * 100)..'%'
    --         end
    --     end
    -- -- when there is no multiplier info, only number of hits
    -- elseif (numberOfHits > 1) then
    --     comboIcon = comboIcon..''..numberOfHits..'x'
    -- end
...
local function buildComboRow(comboTable, comboType)
    ...
    local comboRow = 
    '| id="'..comboName..'" style="padding: 0em 0.5em; line-height:100%;" | '
    ...
    elseif (comboType == 'Forward') then
        comboRow = comboRow..comboName..'[[Stance|*]]'
    ...
    elseif (comboType == 'Heavy') then
        ...
        comboRow = comboRow..'[[Melee 2.0#Heavy Attacks|'..comboName..']]'
    elseif (comboType == 'Slide') then
        comboRow = comboRow..'[[Melee 2.0#Slide Attack|'..comboName..']]'
    elseif (comboType == 'Air') then
        comboRow = comboRow..'[[Melee 2.0#Aerial Attack|'..comboName..']]'
    elseif (comboType == 'Wall') then
        comboRow = comboRow..'[[Melee 2.0#Wall Attack|'..comboName..']]'
    elseif (comboType == 'Ground') then
        comboRow = comboRow..'[[Finisher#Ground Finisher|'..comboName..']]'
    end
    ...
    elseif (comboType == 'ForwardBlock') then
        comboRow = comboRow..'( '..p.addComboIcon('Up', 'Default', nil, nil, nil)..'+  '..p.addComboIcon('Block', 'Default', nil, nil, nil)..')( '
    elseif (comboType == 'Block') then
        comboRow = comboRow..p.addComboIcon('Block', 'Default', nil, nil, nil)..'( '
    elseif (comboType == 'Slide') then
        comboRow = comboRow..'[[Maneuvers#Sliding|Slide]] + '
    elseif (comboType == 'Air') then
        comboRow = comboRow..'[[Maneuvers#Jumping|In Air]] + '
    elseif (comboType == 'Wall') then
        comboRow = comboRow..'[[Maneuvers#Wall Dash|Wall Dash]] + '
    elseif (comboType == 'Ground') then
        comboRow = comboRow..'[[Knockdown|Knocked Down]] Enemy + '
    end
...
local function buildAttackTypeLegendRow()
    return
    '! colspan="2"  style="text-align:center; padding: 0.2em 0.5em 0.125em 0.5em;" | '..p.addComboIcon('Default', '360', nil, nil, nil)..' 360° Attack  '..p.addComboIcon('Default', 'Slam', nil, nil, nil)..' Slam Attack  '..p.addComboIcon('Default', 'Proc', nil, nil, nil)..' Status Proc\n'..
    '|-\n'
end
...
\n'..
    ...
    '| colspan="1"  style="text-align:left; padding: 0.3em 0.1em 0em;" | '..p.addComboIcon('Default', 'Default', nil, nil, nil)..'   Melee   \n'..
    '|-\n'..
    '| colspan="1"  style="text-align:left; padding: 0.3em 0.1em 0em;" | '..p.addComboIcon('Block', 'Default', nil, nil, nil)..'   [[Melee#Blocking|Block]]   \n'..
    '|-\n'..
    '| colspan="1"  style="text-align:left; padding: 0.3em 0.1em 0em;" | '..p.addComboIcon('Up', 'Default', nil, nil, nil)..'   Forward   \n'..
    '|-\n'..
    '| colspan="1"  style="text-align:left; padding: 0.3em 0.1em 0em;" | '..p.addComboIcon('Heavy', 'Default', nil, nil, nil)..'   Heavy Attack   \n'..
    '|}
...
\n'..
    '{| style="margin: -0.5em 0em .8em 0.2em" |\n'..
    '| colspan="1"  style="text-align:left; padding: 0.3em 0.1em 0.65em;" | '..p.addComboIcon('Default', 'Default', 2, nil, nil)..'   Attack does double damage   \n'..
    '|-\n'..
    '| colspan="1"  style="text-align:left; padding: 0.3em 0.1em 0.65em;" | '..p.addComboIcon('Default', 'Default', nil, 2, nil)..'   Attack hits twice   \n'..
    '|}
...
\n'..
    ...
    '| colspan="1"  style="text-align:left; padding: 0.15em 0.2em 0.15em;" | '..tooltip.getFullTooltip("Knockdown", "DamageTypes")..'   Knockdown   \n'..
    '|-\n'..
    '| colspan="1"  style="text-align:left; padding: 0.15em 0.2em 0.15em;" | '..tooltip.getFullTooltip("Ragdoll", "DamageTypes")..'   Ragdoll   \n'..
    '|-\n'..
    '| colspan="1"  style="text-align:left; padding: 0.15em 0.2em 0.15em;" | '..tooltip.getFullTooltip("Stagger", "DamageTypes")..'   Stagger   \n'..
    '|-\n'..
    '| colspan="1"  style="text-align:left; padding: 0.15em 0.2em 0.15em;" | '..tooltip.getFullTooltip("Lifted", "DamageTypes")..'   Lifted   \n'..
    '|-\n'..
    '| colspan="1"  style="text-align:left; padding: 0.15em 0.2em 0.15em;" | '..tooltip.getFullTooltip("Finisher", "DamageTypes")..'   Open to [[Finisher]]s   \n'..
    '|-\n'..
    '| colspan="1"  style="text-align:left; padding: 0.15em 0.2em 0.15em;" | '..tooltip.getFullTooltip("Impair", "DamageTypes")..'   Impair ([[Conclave|PvP]] only)   \n'..
    '|}
...
function p.buildTable(frame)
    ...
    -- if (stanceName == '') then
    --     return 'Error: Stance name argument needed'
    -- end
    ...
    if (STANCES_DATA[stanceName] == nil) then
        return 'Error: Stance name not found'
    end
