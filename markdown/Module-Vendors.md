---
title: "Module:Vendors"
wiki_url: "https://wiki.warframe.com/w/Module/Vendors"
wiki_timestamp: "2023-07-16T12:33:07Z"
---
...
:   Builds lists of sources with vendors where an item can be purchased from. Used in infobox builders such as [Module:Mods/infobox](/w/Module:Mods/infobox "Module:Mods/infobox")
...
:   Builds a list of [Varzia](/w/Varzia "Varzia") relics offerings for display on her page and on [Aya](/w/Aya "Aya"). At the time of writing this function is merely an excuse to put Varzia's relics *somewhere* into Lua so that it can be queried later on the relic own page. Ideally, this should handle all the offerings, but as the comment at the top of [Module:Vendors/data](/w/Module:Vendors/data "Module:Vendors/data") alludes, that is blocked on creating a sort of database for the offerings images. So maybe some other time.
...
* [Vendors/data](/w/Module:Vendors/data "Module:Vendors/data")
* [Vendors/data/doc](/w/Module:Vendors/data/doc "Module:Vendors/data/doc")
* [Vendors/doc](/w/Module:Vendors/doc "Module:Vendors/doc")
...
| Vendors | [M:Baro](/w/Module:Baro "Module:Baro") ([/data](/w/Module:Baro/data "Module:Baro/data")) • M:Vendors ([/data](/w/Module:Vendors/data "Module:Vendors/data")) |
...
function p._buildVendorSourceStrings(name)
	...
	return table.concat(vendorList, '  
')
end
...
'
	...
	for _, offering in pairs(vendorOfferings) do
		...
		-- 	)
		local offeringBox = string.format('%s.png|link=%s|[[%s|%s]]%s  
%s  
(%s)',
				name:gsub('%s+', ''),
				name,
...
function p.varziaRelicOfferings(frame)
 ...
 if offer[OFFERING\_TYPE\_COL] == "Relic" then
 table.insert(relics, string.format("\*{{Relic|%s}}", offer[OFFERING\_NAME\_COL]) )
 end
 end
 local out = table.concat(relics, "  
\n")
