...
wiki_url: "https://wiki.warframe.com/w/Module/DropTables"
wiki_timestamp: "2026-03-08T09:20:46Z"
---
...
* [Template:RelicTable](/w/Template:RelicTable "Template:RelicTable")
* [Template:DropLocations](/w/Template:DropLocations "Template:DropLocations")
...
`droptables._getMissionDropTable(missionAlias)` (function)
:   Gets mission drop table by Alias.
:   **Parameter**: `missionAlias` Alias key value as seen in `/data` (string)
:   **Returns**: Mission drop table entry from `/data` (table)

`droptables._getMissionTable(missionDropTableAlias)` (function)
:   Gets the list of missions that give rewards for a specific Alias (e. g. "Defense1").
:   **Parameter**: `missionDropTableAlias` (string)
:   **Returns**: List of mission nodes with the specified drop table (table)

`droptables.getMissionsForPlanet(planetName)` (function)
:   Gets a list of missions with rewards for a given planet.
:   **Parameter**: `planetName` Name of planet (string)
:   **Returns**: List of mission node table entries as seen in M:Missions/data (table)

`droptables.getRewardTable(frame)` (function)
:   Returns the rewards for a given mission/tier Returns as rows for a table with six columns, two for each rotation See existing Survival/Rewards/Normal\_Mission for examples if Tier==AllTier it will call a specific function to merge all tiers together in a single A, B, C table Used on [Template:Rewards](/w/Template:Rewards "Template:Rewards")
:   **Parameter**: `frame` Frame object (table)
:   **Returns**: Resultant wikitext of wikitable (string)

`droptables.getMissionList(frame)` (function)
:   Gets a list of all the missions for a given Alias. Used on [Template:Rewards](/w/Template:Rewards "Template:Rewards") and p.getSingleRelicByLocation()
:   **Parameter**: `frame` Frame object (table)
:   **Returns**: Wikitext of resultant list (string)

`droptables.getRelicTable(frame)` (function)
:   Returns a wikitable of all mission types and the possible Void Relics they can drop. Used on [Void Relic/ByMission](/w/Void_Relic/ByMission "Void Relic/ByMission") Unlike p.getRewardTable(), this is just the full table with all formatting This is pretty ugly, but kinda have to do it this way (Unless you have a better solution, in which case by all means go ahead and fix it) (I'm not exactly a Lua expert or a UI expert) TODO: Break up this function into smaller functions
:   **Parameter**: `frame` Frame object (table)
:   **Returns**: Wikitext of resultant wikitable (string)

`droptables.getRelicByLocation(frame)` (function)
:   Returns a wikitable of each Void Relic's mission drop locations. Used on [Void Relic/DropLocationsByRelic](/w/Void_Relic/DropLocationsByRelic "Void Relic/DropLocationsByRelic") TODO: Break up this function into smaller functions
:   **Parameter**: `frame` Frame object (table)
:   **Returns**: Wikitext of resultant wikitable (string)

`droptables.getSingleRelicByLocation(frame)` (function)
:   Returns a wikitable with the mission types that drop a particular Void Relic. TODO: Break up this function into smaller functions
:   **Parameter**: `frame` Frame object (table)
:   **Returns**: Wikitext of resultant wikitable (string)

`droptables.getItemByMissionTable(frame)` (function)
:   Returns a wikitable of missions that drop a particular item. Used in [Template:DropLocations](/w/Template:DropLocations "Template:DropLocations")
:   **Parameter**: `frame` Frame object (table)
:   **Returns**: Wikitext of resultant wikitable (string)

`droptables.getItemByEnemyTable(frame)` (function)
:   Returns a wikitable of enemies that drop a particular item. Used in [Template:DropLocations](/w/Template:DropLocations "Template:DropLocations")
:   **Parameter**: `frame` Frame object (table)
:   **Returns**: Wikitext of resultant wikitable (string)

`droptables.getItemByEnemyCount(frame)` (function)
:   Returns the number of enemies that drop a particular item.
:   **Parameter**: `frame` Frame object (table)
:   **Returns**: Number of enemies that drop a particular item (number)

`droptables.getItemByMissionCount(frame)` (function)
:   Returns the number of missions that drop a particular item.
:   **Parameter**: `frame` Frame object (table)
:   **Returns**: Number of missions that drop a particular item (number)

`droptables.getFullEnemyList(frame)` (function)
:   Returns a list of all enemies with a drop table as stored in `/data` TODO: Remove this function once we populate [Module:Enemies/data](/w/Module:Enemies/data "Module:Enemies/data")
:   **Parameter**: `frame` Frame object (table)
:   **Returns**: Wikitext of resultant list (string)

`droptables.getEnemyModDrops(frame)` (function)
:   Returns a list of mod drops from a particular enemy. Used in [Template:EnemyHoriz](/w/Template:EnemyHoriz "Template:EnemyHoriz")
:   **Parameter**: `frame` Frame object (table)
:   **Returns**: Wikitext of resultant list (string)
...
* [DropTables/JSON](/w/Module:DropTables/JSON "Module:DropTables/JSON")
...
* [DropTables/data](/w/Module:DropTables/data "Module:DropTables/data")
* [DropTables/data/dev](/w/Module:DropTables/data/dev "Module:DropTables/data/dev")
* [DropTables/data/doc](/w/Module:DropTables/data/doc "Module:DropTables/data/doc")
* [DropTables/dev](/w/Module:DropTables/dev "Module:DropTables/dev")
* [DropTables/dev/doc](/w/Module:DropTables/dev/doc "Module:DropTables/dev/doc")
* [DropTables/doc](/w/Module:DropTables/doc "Module:DropTables/doc")
...
| [Drop Tables](/w/Drop_Tables "Drop Tables") | [M:Acquisition](/w/Module:Acquisition "Module:Acquisition") ([/data](/w/Module:Acquisition/data "Module:Acquisition/data")) • M:DropTables ([/data](/w/Module:DropTables/data "Module:DropTables/data")) • [M:Void](/w/Module:Void "Module:Void") ([/data](/w/Module:Void/data "Module:Void/data")) |
...
--	@param			{table} dropTable Enemy drop table entry as as seen in /data
...
--	@param			{string} missionAlias Alias key value as seen in /data
--	@return			{table} Mission drop table entry from /data
...
--	@param			{table} missionDropTable Mission drop table entry as seen in /data
...
local function formatDropString(drop)
	...
	elseif (dropType == "Relic") then
		...
		-- Mimicking frame object in order to replace displayed text with " (Radiant)"
...
--	@param			{table} mission Mission table entry as seen in /data
...
! [[Star Chart]] Nodes]=] }
	...
	for i = 1, Table.size(relicData.Drops), 1 do
		...
		if types[i] ~= '' then
			...
			local mType = types[i]
			mType = string.gsub(mType, " %(Steel Path%)", "  
%(Steel Path%)")
...
or  
Drop Table]]
! data-sort-type="numeric" | ChanceChance to roll item within drop table
! data-sort-type="numeric" | QuantityAmount rewarded on successful roll
! data-sort-type="numeric" | Avg. per roll(Quantity × Chance)
! [[Star Chart]] Nodes
|-
]=] }
...
! Enemy
! data-sort-type="numeric" | Drop Table ChanceChance to roll drop table
! data-sort-type="numeric" | Item ChanceChance to roll item within drop table
! data-sort-type="numeric" | Chance(Drop Table Chance × Item Chance)
! data-sort-type="numeric" | Expected Kills(1 / Chance), see [[WARFRAME Wiki:Expected & Nearly Guaranteed Numbers]] for more details
! data-sort-type="numeric" | QuantityAmount rewarded on successful roll
! data-sort-type="numeric" | Avg. per roll attempt(Quantity × Chance), average item quantity on a roll attempt (successful or not)
]=] }
...
---	Returns a list of all enemies with a drop table as stored in /data
