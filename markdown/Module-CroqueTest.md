---
title: "Module:CroqueTest"
wiki_url: "https://wiki.warframe.com/w/Module/CroqueTest"
wiki_timestamp: "2021-08-25T16:29:53Z"
---

*Documentation for this module may be created at [Module:CroqueTest/doc](/w/Module:CroqueTest/doc?action=edit&redlink=1 "Module:CroqueTest/doc (page does not exist)")*
    ...
    for j, key in ipairs(keywords) do
        ...
        for jj, key2 in ipairs(antialiases) do 
            ...
            if (string.find(string.upper(line),string.upper(key2))~=nil) then
                ...
                --    if (aliindex[1]>=antiindex[1] and aliindex[1]                --        table.remove(indexes,kk)
                --    end
                --end
...
function p.formatBFS(frame)
    ...
    for i, line in ipairs(BFS) do
        ...
        if(line=="") then
            if (openframe) then 
                result=result..tab..tab..'{tags={""},text={"<%br/>"}},  
'
            end
...
'--
        elseif(string.find(line,"|update number")~=nil) then
            ...
            result=result..'["update number"]={"'..vers..'"},  
'
...
'
            ...
            nline=nline+1
            result=result..tab..tab
            result=result..'{tags={'..tags..'},text={"'..line..'"}},  
'
        end
        
    end
    local preamble="local PatchData =  
{  
"
    local postamble="  
}  
...
function p.NightWaveActs(frame)
    ...
    while(string.find(worldstring,"/Lotus/Types/Challenges/Seasons/Daily/",Index+1)~=nil) do
        ...
        if Acts[ActCode]~=nil then
            localAct["Icon"]="[[File:"..Acts[ActCode]["Icon"].."|center|75px]]"
        ...
        if string.find(localAct["Icon"],"xyz.png")~=nil then localAct["Icon"] = '--[[File:xyz.png|center|75px]] -->' end 
    ...
    local result='{| class="emodtable" style="width:100%;"  
'
    result=result..'|-  
'
    result=result..'!Icon  
...
'
    ...
    local resultDaily=result..'!End time  
'..'|-'
    local resultWeekly=result..'|-'
    local resultEliteWeekly=result..'|-'
    
    for _, lAct in pairs(Daily) do
        resultDaily=resultDaily.."  
|"..lAct["Icon"]
        resultDaily=resultDaily.."  
|'''"..lAct["Name"].."'''/>''"..lAct["Description"].."''"
        if (Automatic == true) then
            resultDaily=resultDaily.."  
|[[File:ReputationLargeBlack.png|20px|link=Syndicates]]"..'‍'..lAct["Standing"]..''
        else
            resultDaily=resultDaily.."  
|{{sc|"..lAct["Standing"].."}}"
        end
        if (Automatic == true) then 
            resultDaily=resultDaily.."  
|"..CDstart..lAct["Expiry"]..CDend.."  
|-"
        else
            resultDaily=resultDaily.."  
|{{NightwaveCountdown|"..lAct["ExpiryTemplate"].."}}  
|-"
        end
...
"
    for _, lAct in pairs(EliteWeekly) do
        resultEliteWeekly=resultEliteWeekly.."  
|"..lAct["Icon"]
        resultEliteWeekly=resultEliteWeekly.."  
|'''"..lAct["Name"].."'''/>''"..lAct["Description"].."''"
        if (Automatic == true) then
...
"
    ...
    if (Automatic==true) then
        if (DWE=="Daily") then
            result = string.gsub(resultDaily,"","")
            result = string.gsub(result,"  
","\n")
        elseif (DWE=="Weekly") then
            result = string.gsub(resultWeekly,"","")
            result = string.gsub(result,"  
","\n")
        elseif (DWE=="EliteWeekly") then
            result = string.gsub(resultEliteWeekly,"","")
            result = string.gsub(result,"  
","\n")
        end
        return result
    else 
        result = '<div class="tabbertab-borderless"><tabber>  
|-|♦ Daily=  
'..resultDaily..'|-|♦♦ Weekly=  
'..resultWeekly..'|-|♦♦♦ Elite Weekly=  
'..resultEliteWeekly..">>"
        return '
